...
import { 
  ...
  ChevronRight, ChevronDown, UserCircle, Globe, FileText, Menu
} from 'lucide-react';
...
type AdminSection = 'about' | 'stores' | 'custom' | 'settings' | 'profile';
...
export default function Admin({ onNavigate }: AdminProps) {
  ...
  const loadData = async () => {
    try {
      const [aboutUsResult, storesResult, settingsResult, customSectionsResult] = await Promise.all([
        supabase.from('about_us').select('*').maybeSingle(),
        supabase.from('stores').select('*').order('orden', { ascending: true }),
        supabase.from('site_settings').select('*'),
        supabase.from('custom_sections').select('*').order('orden', { ascending: true }),
      ]);
      ...
      if (customSectionsResult.data) setCustomSections(customSectionsResult.data);
    } catch (error) {
  ...
  const saveCustomSection = async (section: Partial<CustomSection>) => {
    setLoading(true);
    try {
      console.log('Saving custom section:', section);
      ...
      if (section.id) {
        ...
        const { data, error } = await supabase
          ...
          .update({
            ...
            tipo: section.tipo,
            url_video: section.url_video,
            url_imagen: section.url_imagen,
        ...
        const { data, error } = await supabase.from('custom_sections').insert({
          titulo: section.titulo || '',
          contenido: section.contenido || '',
          tipo: section.tipo || 'text',
          url_video: section.url_video || '',
          url_imagen: section.url_imagen || '',
          visible: section.visible !== false,
          orden: section.orden || customSections.length,
        }).select();
      ...
      showMessage('Error al guardar sección: ' + (error?.message || 'Error desconocido'), 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const CustomSectionForm = ({ section, onSave, onCancel }: { 
    ...
    onSave: (s: Partial<CustomSection>) => void;
    onCancel: () => void;
  }) => {
    const [formData, setFormData] = useState<Partial<CustomSection>>(section);

    return (
      <div className="p-4 bg-slate-950/50 border border-slate-700 rounded-xl space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1">Tipo de contenido</label>
            <select
              value={formData.tipo || 'text'}
        ...
        {formData.tipo === 'text' && (
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1">Contenido</label>
            <textarea
              value={formData.contenido || ''}
              onChange={(e) => setFormData({ ...formData, contenido: e.target.value })}
              className="w-full px-4 py-2 bg-slate-900 border border-slate-700 rounded-lg text-slate-100 h-32"
        ...
        {formData.tipo === 'video' && (
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1">URL del Video (YouTube)</label>
            <input
              type="text"
        ...
        {formData.tipo === 'html' && (
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1">Código HTML</label>
            <textarea
              value={formData.contenido || ''}
              onChange={(e) => setFormData({ ...formData, contenido: e.target.value })}
              className="w-full px-4 py-2 bg-slate-900 border border-slate-700 rounded-lg text-slate-100 h-32 font-mono text-sm"
              placeholder="<div>...</div>"
            />
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="flex items-center">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span className="text-slate-300">Visible en el sitio</span>
            </label>
          </div>
        </div>
  ...
  const menuItems = [
    ...
    { id: 'custom', label: 'Secciones Personalizadas', icon: FileText },
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row">
      ...
      <div className="md:hidden bg-slate-900 border-b border-slate-800 p-4 flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-yellow-400">ADMIN</h2>
          <p className="text-xs text-slate-400">La Alianza</p>
        </div>
        <button 
      ...
      {/* Sidebar */}
      <aside className={`fixed md:static inset-y-0 left-0 z-50 w-64 bg-slate-900 border-r border-slate-800 flex-shrink-0 transform transition-transform duration-300 ${
        sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
      }`}>
        <div className="p-4 md:p-6 border-b border-slate-800">
          <h2 className="text-lg md:text-xl font-bold text-yellow-400">ADMIN</h2>
          <p className="text-xs md:text-sm text-slate-400">La Alianza Carnicerías</p>
        </div>
        ...
        <nav className="p-3 md:p-4">
          {menuItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-2 md:gap-3 px-3 md:px-4 py-2 md:py-3 rounded-lg mb-1 md:mb-2 transition-all text-sm md:text-base ${
        ...
        <div className="p-3 md:p-4 mt-auto border-t border-slate-800">
          <button
            onClick={() => onNavigate('home')}
            className="w-full flex items-center gap-2 px-3 md:px-4 py-2 text-slate-300 hover:text-white text-sm"
          >
            ← Inicio
          </button>
        </div>
      ...
      {/* Main Content */}
      <main className="flex-1 p-3 md:p-8 overflow-auto w-full">
        {message && (
          <div
        ...
        {activeSection === 'custom' && (
          <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-slate-50">Secciones Personalizadas</h2>
              <button
            ...
            <p className="text-slate-400 mb-4">
              Agrega secciones personalizadas debajo de las tiendas (texto, video, imagen, etc.)
            </p>
            ...
            {isAddingCustomSection && (
              <CustomSectionForm
                ...
                onCancel={() => setIsAddingCustomSection(false)}
              />
            )}
            ...
            <div className="space-y-4">
              {customSections.length === 0 ? (
                <p className="text-slate-500 text-center py-8">
                  No hay secciones personalizadas. Agrega una para mostrar contenido debajo de las tiendas.
                </p>
              ) : (
                customSections.map((section) => (
                  <div key={section.id}>
                    {editingCustomSection?.id === section.id ? (
                      <CustomSectionForm
                        section={editingCustomSection}
                        onSave={saveCustomSection}
                        onCancel={() => setEditingCustomSection(null)}
                      />
                    ) : (
                      <div className="flex items-center justify-between p-4 bg-slate-950/30 border border-slate-800 rounded-xl">
                        <div>
                          <h3 className="font-bold text-slate-50">{section.titulo || 'Sin título'}</h3>
                          <p className="text-sm text-slate-300">Tipo: {section.tipo}</p>
                          <p className="text-xs text-slate-400">
                            Orden: {section.orden} | {section.visible ? 'Visible' : 'Oculto'}
                          </p>
                        </div>
                        <div className="flex gap-2">
                          <button onClick={() => setEditingCustomSection(section)} className="p-2 bg-blue-500 rounded-lg">
                            <Edit2 size={18} />
                          </button>
                          <button onClick={() => deleteCustomSection(section.id)} className="p-2 bg-red-500 rounded-lg">
                            <Trash2 size={18} />
                          </button>
                        </div>
                      </div>
